...
def main():
    ...
    for frameid in range(len(img_keys)):
            ...
            for ann in anns:
                if 'keypoints' in ann and type(ann['keypoints']) == list:
                    ...
                    for i in range(x.shape[0]):
                        cv2.putText(img, "{}".format(i), (x[i]+1, y[i]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1, 4)
                        cv2.circle(img, (x[i], y[i]), 2, (0, 0, 255), lineType=1)
        ...
        if save_img:
            print(frameid)
            cv2.imwrite(os.path.join(args.out_img_root, f'vis_{frameid}.jpg'), img)
